...
const SKIN_OPTIONS = ['좋음', '건조', '민감', '습진', '붉은반점', '붉음', '탈모', '딱지', '각질', '기름짐', '기타'] as const
...
export default function EditRecordPageWrapper() {
  return (
    <Suspense fallback={<div className="py-10 text-center text-sm" style={{ color: '#888' }}>불러오는 중...</div>}>
      <EditRecordForm />
    </Suspense>
  )
}
...
function EditRecordForm() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setSessionDate(d.visit_date ?? today)
      setMainService(d.service ?? '')
      setSpaLevel(d.spa_level ?? null)
      ...
      if (d.grooming_style && typeof d.grooming_style === 'object') {
        ...
        setGroomingStyle({
          face: gs.face ?? '', body: gs.body ?? '', legs: gs.legs ?? '',
          tail: gs.tail ?? '', sanitary: gs.sanitary ?? '',
        })
      }

      // ─── 미용 소요시간 ───
      if (typeof d.grooming_duration_minutes === 'number') {
        setGroomingDurationMin(d.grooming_duration_minutes)
      }
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(async () => {
      ...
      for (const g of gRes.data ?? []) {
        if (!results.some((r) => r.guardianId === g.id)) {
          results.push({
            petId: '',
            petName: '',
            breed: null,
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {dataLoading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: '#FFFFFF' }}>
          <div className="text-center">
            <p style={{ fontSize: 15, letterSpacing: '0.35em', fontWeight: 600, color: '#0A0A0A' }}>DAZUL</p>
            <p className="mt-3" style={{ fontSize: 12, color: '#888' }}>불러오는 중...</p>
          </div>
        </div>
      )}
      {loadError && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: '#FFFFFF' }}>
          <div className="text-center">
            <p style={{ fontSize: 14, color: '#C9A96E' }}>{loadError}</p>
            <Link href="/admin/records" className="mt-4 inline-block" style={{ fontSize: 12, color: '#888' }}>← 기록 목록</Link>
          </div>
        </div>
      )}
      ...
      {/* 페이지 타이틀 */}
      <div>
        <h1 className="text-[28px] font-light tracking-[0.05em] text-[#0A0A0A]">케어 기록 수정</h1>
